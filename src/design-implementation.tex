...
However, SPARK requires that functions be purely functional; that is, they cannot have parameters with a mode of `out' or `in out'.
...
\end{lstlisting}
% TODO:
% mention separating into parts relating to IO (disk), and parts not related. then formally verifying not related parts.
% also discuss issues with formally verifying disk IO, also compared to a disk image file.
